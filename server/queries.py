...
def query_films_ranked_higher(limit):
    return f"select film.id as id, film.title as title, (select round(avg(rank),1) from ranking where ranking.film_id = film.id) as rank from film order by rank desc;"
